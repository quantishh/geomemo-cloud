...
def list_events(past: bool = Query(False, description="Include past events")):
    ...
    try:
        if past:
            cursor.execute("""
                SELECT id, title, url, location, start_date, end_date,
                       description, category, created_at::text
                FROM events
...
def create_event(event: EventCreate):
    ...
    try:
        cursor.execute("""
            INSERT INTO events (title, url, location, start_date, end_date, description, category)
            VALUES (%s, %s, %s, %s, %s, %s, %s)
            RETURNING id, title, url, location, start_date, end_date,
                      description, category, created_at::text
        """, (
